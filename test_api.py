...
def test_getUser(app_context):
    ...
    #test response
    assert response.status_code==200
    assert json.loads(response.data)==({'Name':user_test_name, 'Public_ID':user_test_public_ID,'Email':user_test_email})
...
def test_deleteUser(app_context):
    ...
    assert response.status_code==200
    assert json.loads(response.data)==f'deleted information for user with ID: {user_test_public_ID}'
...
def test_getMachineById(app_context):
    ...
    db.session.commit()
    assert response.status_code == 200
    assert json.loads(response.data)==({'ID':1, 'Floor_ID':1,'Floor': 0,'Dorm':'Sittner','Is_Available': True,'Last_Service_Date':'10/27/2022', 'Installation_Date':'10/27/2022'})
...
def test_deleteMachineById(app_context):
    ...
    Machine.query.filter_by(id = machine_test_id).delete()
    assert response.status_code==200
    assert json.loads(response.data)==f'deleted information for machine with ID: {machine_test_id}'
...
def test_createMachineById(app_context):
    ...
    db.session.commit()
    assert response.status_code==200
    assert json.loads(response.data)==f'created information for machine with ID: {machine_test_id}'
...
def test_getMachinesByDorm(app_context):
    ...
    db.session.commit()
    assert response.status_code == 200
    assert json.loads(response.data)==[[1,1,1,True],[2,2,2,True],[3,3,3,True]]
